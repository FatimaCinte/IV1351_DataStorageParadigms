...
CREATE TABLE "time_period" (
  ...
  start timestamp(5) NOT NULL,
  finish timestamp(5) NOT NULL
);

CREATE TABLE "price_scheme" (
  price_scheme_id SERIAL PRIMARY KEY,
  beginner_price varchar(10) NOT NULL, 
  intermediate_price varchar(10) NOT NULL, 
  advanced_price varchar(10) NOT NULL, 
  individual_price varchar(10) NOT NULL, 
  group_price varchar(10) NOT NULL, 
  ensemble_price varchar(10) NOT NULL, 
  instrument_rental varchar(10) NOT NULL,  
  sibling_discount varchar(10) NOT NULL, 
  instructor_salary_percentage varchar(10) NOT NULL,
...
CREATE TABLE "instrument" (
  ...
  FOREIGN KEY(instrument_type_id) REFERENCES instrument_type(instrument_type_id) ON DELETE SET NULL
); 

CREATE TABLE "instrument_for_rental" (
  instrument_for_rental_id SERIAL PRIMARY KEY,
  date_of_rental varchar(5) NOT NULL,
  date_to_return varchar(5) NOT NULL,
...
CREATE TABLE "instructor_time_period" (
  instructor_id INT,
  time_period_id INT,
  PRIMARY KEY(instructor_id, time_period_id),
  FOREIGN KEY(instructor_id) REFERENCES instructor(instructor_id) ON DELETE CASCADE,
  FOREIGN KEY(time_period_id) REFERENCES time_period(time_period_id) ON DELETE CASCADE
);
